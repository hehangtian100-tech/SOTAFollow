...
3. **Cross-Attention 时序融合**：替代自注意力，复杂度从 O(H²) 降至 O(h×H)，配合 KV Cache 实现线性历史检索
...
> 朴素地扩展历史窗口会让 (a) 性能下降，且 (c) 计算成本极高。GMP 通过选择性记忆召回在三者上均保持高效
> **来源**：arXiv HTML 2604.18933v1
...
> **图 3：Memory Gate 校准流程**（对应论文 Figure 3）
> 四步校准：① 在 D_train 上训练 Gate 常闭/常开两个策略；② 在 D_val 上采样 N 条轨迹并计算各时刻动作预测误差；③ 对比误差比值生成二值标签 μt；④ 冻结 Gate 权重，在完整数据集上重训策略
> **来源**：arXiv HTML 2604.18933v1
...
> - **右 (b)**：Gated Attention 模块三要素：① 二值门控 μt 决定是否执行历史交叉注意力；② 加噪历史动作条件提升鲁棒性；③ KV Cache 缓存历史 token 降低计算成本
> **来源**：arXiv HTML 2604.18933v1（需从论文源码提取实际图片 URL 替换下方占位图）



### 3.5 训练流程与伪代码

GMP 的完整训练分为两个阶段：**Gate 校准阶段**和**策略微调阶段**。
...
# δ_t  = 无记忆策略在时刻 t 的动作预测误差
# δ_t^mem = 有记忆策略在时刻 t 的动作预测误差
errors_off = compute_errors(trajectories_off)   # shape: [T_val]
errors_on  = compute_errors(trajectories_on)     # shape: [T_val]
...
# 使用校准好的 Gate（冻结），在完整数据集上训练策略
pi_gated = train_policy(D_full, gate_mode="calibrated", gate_mlp=gate_mlp)

# 训练目标
for batch in D_full:
    I_t, P_t, A_t:t+h, k = batch["image"], batch["proprio"], batch["actions"], batch["diffusion_step"]
...
> **图 4 / 图 8：Cross-Attention 注意力可视化**（对应论文 Figure 4、Figure 8）
> - **左图（Match Color）**：t=80 时刻（放置立方体），注意力权重最高点落在 t=48（首次观察箱子颜色），说明模型自动学会追踪关键视觉帧
> - **右图（Iterative Pushing）**：第 4 次推动时，注意力集中在第 2、3 次推的结果（overshoot / undershoot），跨 Trial 推断物理动态
> 蓝色 = μt=1（门开），灰色 = μt=0（门关）；无监督下习得最有信息量的历史时刻
> **来源**：arXiv HTML 2604.18933v1
