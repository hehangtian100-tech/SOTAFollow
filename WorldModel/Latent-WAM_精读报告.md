...
2. 随机初始化场景查询 $Q_{scene} \in ℝ^{T \times M \times N \times D_e}$，与 $X$ 拼接后送入 DINO encoder $ℰ$（含 MLP 投影到 $D_l = 256$ 维潜在空间）：

\(\hat{Q}_{scene}, \hat{X} = ℰ([Q_{scene}; X])\)
...
\(L_{align} = 1 - \cos(LN(\phi(\hat{X})), LN(f_g(I)))\)
...
1. **Ego 状态编码器**（单层 MLP）：将驾驶命令、速度、加速度编码为 $S_{ego} \in ℝ^{T \times D_l}$

2. **Scene-Ego 融合**：多相机 scene tokens 聚合为全局感知表示，拼接 $S_{ego}$，得到统一的世界状态：

\(S_{world} \in ℝ^{T \times (M \times N + 1) \times D_l}\)

#### 3.3.2 因果世界模型预测

**核心思路**：将世界状态转移建模为**自回归预测问题**，用标准 next-token prediction 训练。

1. 随机初始化未来世界状态查询 $Q_{future} \in ℝ^{(T-1) \times (M \times N + 1) \times D_l}$

2. 历史世界状态 $S_{world}^{i}, i \in \{1, \ldots, T-1\}$ 构成 KV cache

3. DLWM（带 RoPE 的标准 Transformer 解码器）预测未来状态：

\(S_{future} = DLWM(Q_{future}, KV_{future})\)
...
**问题**：$Q_{future}$ 和 $KV_{future}$ 未携带明确的时序/空间位置信息。
...
- 驾驶命令：$\hat{C} = Softmax(D_{cmd}(S_{ego}^{i'}))$
- 速度：$\hat{V} = D_v(S_{ego}^{i'})$
- 加速度：$\hat{A} = D_a(S_{ego}^{i'})$

损失：$L_{ego} = L_{cmd} + L_v + L_a$

### 3.4 轨迹规划

**输入**：可学习的轨迹查询 $Q_\tau \in ℝ^{K \times n_p \times D_l}$ + 当前世界状态 $S_{world}^t$ + 当前驾驶命令 $C$

**输出**：$K$ 条候选轨迹，每条 $n_p$ 个 pose $(x, y, \theta)$，根据 $C$ 选择对应候选作为最终轨迹 $\tau$：

\(\tau = D_\tau(Q_\tau, S_{world}^t, C)\)
...
- $L_{traj}$：L1 损失（模仿专家轨迹）
- $L_{align}$：cosine 相似度损失（几何蒸馏）
- $L_{wm}$：MSE 损失（未来世界状态预测）
- $L_{ego}$：交叉熵 + MSE（Ego 状态监督）
...
\(EPDMS = NC \times DAC \times DDC \times TLC \times (5 \times (EP + TTC) + 2 \times (LK + HC + EC))/(16)\)

新增 DDC（驾驶方向合规）、TLC（交通灯合规）、LK（车道保持）、HC（历史舒适性）、EC（扩展舒适性）指标。

#### HUGSIM HD-Score 公式

\(HD-Score_t = NC \times DAC \times (5 \times TTC + 2 \times COM)/(7)\)

最终 HD-Score = $R_c \times (1)/(T)∑_{t=0}^{T} HD-Score_t$
