...
**输入**：时序多视角图像 $I \in \mathbf{R}^{\mathbf{T} \times \mathbf{M} \times H \times W \times 3}$

1. 图像嵌入为 patch tokens：$X \in \mathbf{R}^{\mathbf{T} \times \mathbf{M} \times \mathbf{S} \times D_e}$
   - $\mathbf{T}$：时序帧数
   - $\mathbf{M}$：相机数量
   - $\mathbf{S}$：每张图的 patch 数
   - $D_e = 768$

2. 随机初始化场景查询 $Q_{\text{scene}} \in \mathbf{R}^{\mathbf{T} \times \mathbf{M} \times \mathbf{N} \times D_e}$，与 $X$ 拼接后送入 DINO encoder $\mathcal{E}$（含 MLP 投影到 $D_l = 256$ 维潜在空间）：
...
1. 多视角图像输入几何基础模型 $f_g$，生成 patch 级几何特征 $f_g(I) \in \mathbf{R}^{\mathbf{T} \times \mathbf{M} \times \mathbf{S} \times D_g}$，其中 $D_g = 2048$

2. DINO backbone 输出的 $\hat{X}$ 通过几何投影器 $\phi$ 投影到同一空间：$\phi(\hat{X}) \in \mathbf{R}^{\mathbf{T} \times \mathbf{M} \times \mathbf{S} \times D_g}$
...
1. **Ego 状态编码器**（单层 MLP）：将驾驶命令、速度、加速度编码为 $S_{\text{ego}} \in \mathbf{R}^{\mathbf{T} \times D_l}$

2. **Scene-Ego 融合**：多相机 scene tokens 聚合为全局感知表示，拼接 $S_{\text{ego}}$，得到统一的世界状态：

\(S_{\text{world}} \in \mathbf{R}^{\mathbf{T} \times (\mathbf{M} \times \mathbf{N} + 1) \times D_l}\)

#### 3.3.2 因果世界模型预测

**核心思路**：将世界状态转移建模为**自回归预测问题**，用标准 next-token prediction 训练。

1. 随机初始化未来世界状态查询 $Q_{\text{future}} \in \mathbf{R}^{(\mathbf{T}-1) \times (\mathbf{M} \times \mathbf{N} + 1) \times D_l}$
...
**输入**：可学习的轨迹查询 $Q_\tau \in \mathbf{R}^{K \times n_p \times D_l}$ + 当前世界状态 $S_{\text{world}}^t$ + 当前驾驶命令 $C$
...
"""
Latent-WAM 训练伪代码
论文 Algorithm 完整实现 + 中文逐行注释

输入: 多视角图像序列 I, 驾驶命令 C, 专家轨迹 τ
输出: SCWE + DLWM + Trajectory Decoder 联合训练
"""

def latent_wam_train(images, commands, expert_traj,
                    scwe, dlwm, traj_decoder,
                    optimizer, config):
    """
    Args:
        images: [T, M, H, W, 3] 多视角图像序列
        commands: [T] 驾驶命令 (left, straight, right)
        expert_traj: [Np, 3] 专家轨迹 (x, y, theta)
        scwe: Spatial-Aware Compressive World Encoder
        dlwm: Dynamic Latent World Model
        traj_decoder: 轨迹解码器
        config: 超参数配置
    """
    # ========== Stage 1: SCWE 场景压缩 ==========
    # 1.1 图像编码为 patch tokens
    patch_tokens = scwe.image_encoder(images)  # [T, M, S, D_e]

    # 1.2 Query-based 压缩 (16 个 scene queries)
    scene_tokens = scwe.scene_queries(patch_tokens)  # [T, M, N, D_l]

    # ========== Stage 2: 几何特征蒸馏 ==========
    # 2.1 几何基础模型提取空间感知特征 (冻结)
    geo_features = scwe.world_mirror(images)  # [T, M, S, D_g]

    # 2.2 几何对齐损失 (cosine similarity)
    align_loss = scwe.compute_align_loss(scene_tokens, geo_features)

    # ========== Stage 3: DLWM 世界状态预测 ==========
    # 3.1 Ego 状态编码
    ego_state = scwe.ego_encoder(commands)  # [T, D_l]

    # 3.2 融合为世界状态
    world_state = scwe.fuse_scene_ego(scene_tokens, ego_state)  # [T, M*N+1, D_l]

    # 3.3 因果世界模型预测 (Teacher Forcing)
    future_queries = dlwm.init_future_queries()  # [T-1, M*N+1, D_l]
    kv_cache = world_state[:-1]  # 历史 KV cache
    pred_future = dlwm.predict(future_queries, kv_cache)  # [T-1, M*N+1, D_l]

    # 3.4 Ego 状态监督
    ego_loss = dlwm.compute_ego_loss(pred_future, ego_state)

    # ========== Stage 4: 轨迹规划 ==========
    # 4.1 可学习轨迹查询
    traj_queries = traj_decoder.init_queries()  # [K, n_p, D_l]

    # 4.2 轨迹解码
    pred_traj = traj_decoder(traj_queries, world_state[-1], commands[-1])

    # 4.3 轨迹损失 (L1)
    traj_loss = F.l1_loss(pred_traj, expert_traj)

    # ========== 总损失与优化 ==========
    total_loss = (traj_loss
                  + config.alpha * align_loss
                  + config.beta * dlwm.world_loss
                  + config.gamma * ego_loss)
    ...
    return {
        ...
        'world_loss': dlwm.world_loss.item(),
        'ego_loss': ego_loss.item()
    }


def latent_wam_inference(images, commands, scwe, dlwm, traj_decoder):
    """
    推理阶段: 纯前向传播，无梯度计算

    Args:
        images: [T, M, H, W, 3] 当前及历史多视角图像
        commands: [T] 驾驶命令
    Returns:
        traj: [n_p, 3] 预测轨迹 (x, y, theta)
    """
    # Stage 1: SCWE 压缩
    patch_tokens = scwe.image_encoder(images)
    scene_tokens = scwe.scene_queries(patch_tokens)
    ego_state = scwe.ego_encoder(commands)
    world_state = scwe.fuse_scene_ego(scene_tokens, ego_state)

    # Stage 2: 轨迹解码 (无需 DLWM 推理!)
    traj_queries = traj_decoder.init_queries()
    pred_traj = traj_decoder(traj_queries, world_state[-1], commands[-1])

    return pred_traj  # [n_p, 3], n_p=8 (4秒/0.5秒)
```

**推理要点**：
- **DLWM 不参与推理**：训练时学习世界动态，推理时仅用 SCWE 压缩 + Trajectory Decoder
- **零额外延迟**：几何特征离线预计算，推理仅需一次前向传播
- **K 条候选轨迹**：根据当前命令 $C$ 选择对应候选作为最终输出
